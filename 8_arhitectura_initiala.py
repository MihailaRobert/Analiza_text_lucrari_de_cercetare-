...
# Se ia datele NECURĂȚATE (toate cele ~14k paragrafe)
...
print("\n" + "="*60)
print(" Începem: ANTRENAMENTUL MODELULUI INIȚIAL (Pe date NECURĂȚATE)")
print("="*60)
print(f"Device: {device}")

if not os.path.exists(input_file):
    print(f"EROARE: Nu găsesc '{input_file}'. Asigură-te că există.")
    exit()

# 2. PREGĂTIRE DATE BRUTE
df = pd.read_csv(input_file)
df = df.dropna(subset=["section", "label"]).copy() # sa nu avem rânduri corupte
...
print(f"--> Antrenăm pe {len(df)} paragrafe NECURĂȚATE. (Clase: {NUM_LABELS})")

MAX_LEN = 128
BATCH_SIZE = 4
EPOCHS = 3 # Am redus la 3 pentru că volumul de date este triplu (scutește CPU-ul)
...
# INGHETAM BAZA BERT PENTRU VITEZA MAXIMA PE CPU
...
# --- 3. FUNCȚIE ANTRENARE ---
def train_initial_model(model, train_loader, optimizer, scheduler):
    ...
    for epoch in range(EPOCHS):
        model.train()
        running_loss = 0.0
        print(f"\n[Model Inițial] Epoca {epoch+1}/{EPOCHS}...")
        ...
        for step, batch in enumerate(train_loader):
            ...
            if step % 100 == 0 and step > 0:
                ...
                print(f"   Pas {step}/{len(train_loader)} | Loss: {loss.item():.4f} | Timp scurs: {elapsed:.0f}s")
...
train_loss_history = train_initial_model(model, train_loader, optimizer, scheduler)

# 4. EVALUARE ȘI EXTRAGERE REZULTATE
print("\n--> Generăm rezultatele și graficele pe setul de validare ")
model.eval()
...
plt.title("Model Inițial (Date Necurățate) - Matrice de Confuzie", fontsize=12)
plt.xlabel("Prezis")
plt.ylabel("Adevărat")
plt.xticks(rotation=45, ha='right')
save_cm = os.path.join(base_path, "8_output_Initial_ConfusionMatrix.png")
...
plt.title(f"BERT Base — Evoluția pierderii (Date Necurățate) pe {EPOCHS} epoci", fontsize=12)
plt.xlabel("Epocă")
plt.ylabel("Pierdere (Loss)")
plt.xticks(epochs_axis)
plt.grid(True, linestyle="--", alpha=0.5)
plt.legend(loc="upper right")
save_loss = os.path.join(base_path, "8_output_Initial_LossCurve.png")
...
plt.title('Curba ROC (Model Inițial Pe Date Necurățate)')
plt.legend(loc="lower right", fontsize=9)
plt.grid(True, linestyle="--", alpha=0.5)

save_roc = os.path.join(base_path, "8_output_Initial_ROC.png")
...
# PRINTĂM RAPORTUL ȘI ÎN CONSOLĂ
print("\n--- Rezultate BERT (Model Inițial pe Date Necurățate) ---")
print(report_str)

plt.figure(figsize=(10, 6))
plt.axis('off')
plt.text(0.01, 0.95, f"Raport Clasificare - Model Inițial (Date Necurățate)\n\n{report_str}", 
         fontfamily='monospace', fontsize=12, verticalalignment='top')
save_text = os.path.join(base_path, "8_output_Initial_Report.png")
plt.savefig(save_text, bbox_inches='tight', dpi=300)
plt.close()

print("\n" + "="*60)
print(" SCRIPT INIȚIAL FINALIZAT! ")
print(" Graficele pentru 'Before' au fost salvate cu prefixul '8_output_Initial_'")
print("="*60)