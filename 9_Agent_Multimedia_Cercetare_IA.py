...
if os.path.exists(CALE_CSV):
    ...
    if articol_selectat:
        ...
        # SECȚIUNEA 11: Determinare elemente principale
        st.subheader("11. Determinarea elementelor principale ale unui articol de cercetare")

        # Specifică sursa datelor 
        NUME_FISIER_SURSA = "1_output_all_articles_paragraph_dataset_with_sections.csv"

        # Gestiune Cache (pentru a reține rezultatele între interacțiuni)
        if "articol_anterior" not in st.session_state:
            st.session_state.articol_anterior = None

        if st.session_state.articol_anterior != articol_selectat:
            st.session_state.cache_tabel_v11 = None
            st.session_state.articol_anterior = articol_selectat

        st.markdown("""
            <style>
                table { width: 100% !important; border-collapse: collapse; margin-top: 20px; }
                th:nth-child(1), td:nth-child(1) { width: 50px !important; text-align: center !important; }
                th:nth-child(2), td:nth-child(2) { width: 220px !important; text-align: center !important; font-weight: bold; }
                th:nth-child(3), td:nth-child(3) { width: 100px !important; text-align: center !important; }
                th:nth-child(4), td:nth-child(4) { text-align: left !important; padding: 20px !important; }
                td { vertical-align: middle !important; border-bottom: 1px solid #ddd; }
                tr:hover { background-color: #f5f5f5; }
            </style>
        """, unsafe_allow_html=True)

        # CONFIGURARE (Cuvinte cheie + Secțiuni din fișierul CSV)
        config_analiza = {
            "Set de date public (Public data set)": {
                "keywords": ["dataset", "data set", "publicly available", "open data", "benchmarks", "data collection"],
                "sections": ["Introduction", "Body", "Proposed Approach"]
            },
            "Cod sursă public (Public code)": {
                "keywords": ["github", "source code", "repository", "implementation", "available at", "scripts", "codebase"],
                "sections": ["Introduction", "Body", "Proposed Approach"]
            },
            "Unelte tehnologice (Tools/Technological)": {
                "keywords": ["framework", "tool", "library", "pytorch", "tensorflow", "scikit", "keras", "python", "software", "opencv", "transformers"],
                "sections": ["Proposed Approach", "Body", "Results"]
            },
            "Resource used": {
                "keywords": ["gpu", "nvidia", "a100", "v100", "tpu", "rtx", "compute", "hardware", "ram", "server", "infrastructure", "workstation"],
                "sections": ["Proposed Approach", "Body", "Results"]
            },
            "Contributions": {
                "keywords": ["contribution", "we propose", "novel", "our method", "we introduce", "key contributions", "this work"],
                "sections": ["Introduction", "Proposed Approach"]
            },
            "Proposed produce (in short)": {
                "keywords": ["summary", "proposed", "approach", "in short", "methodology", "architecture", "system design"],
                "sections": ["Proposed Approach", "Introduction", "Body"]
            },
            "Take home message": {
                "keywords": ["conclusion", "finding", "take home", "final message", "summary", "overall", "highlights", "demonstrate", "in essence"],
                "sections": ["Conclusion", "Results"]
            }
        }

        optiune_unica = st.selectbox("Alege element rapid:", list(config_analiza.keys()))
        col_b1, col_b2 = st.columns([1, 4])
        btn_indiv = col_b1.button(f"🔍 Caută doar '{optiune_unica}'")
        btn_all = col_b2.button("Start Analiză Completă (Toate)")

        spatiu_status = st.empty() 
        spatiu_tabel = st.empty()  

        # PROCESARE DATE DIN FIȘIERUL: 1_output_all_articles_paragraph_dataset_with_sections.csv
        if btn_indiv or btn_all:
            _, _, t_tra, m_tra, dev = incarca_modelele_ia()
            lista_task = [optiune_unica] if btn_indiv else list(config_analiza.keys())
            rezultate_temporare = []
            
            for i, nume in enumerate(lista_task):
                spatiu_status.info(f"Se analizează fișierul **{NUME_FISIER_SURSA}** pentru: **{nume}**...")
                
                cuvinte = config_analiza[nume]["keywords"]
                sectiuni_permise = config_analiza[nume]["sections"]
                
                # Colectare fragmentele relevante extrase din fifierul .csv
                paragrafe_relevante_din_csv = []
                
                # Filtrare bazată pe coloana 'section' din dataset-ul încărcat din CSV
                date_filtrare = date_specifice[date_specifice['section'].isin(sectiuni_permise)]
                if date_filtrare.empty: date_filtrare = date_specifice

                # Parcurge rândurile din fișierul: 1_output_all_articles_paragraph_dataset_with_sections.csv
                for _, rând_csv in date_filtrare.iterrows():
                    # Preluăm textul din coloana 'paragraph_text'
                    p_text = str(rând_csv['paragraph_text']).strip()
                    
                    # Filtru de lungime minimă
                    if len(p_text) < 130: continue
                    
                    # CALCUL SCOR 
                    scor_paragraf = 0
                    for cuvânt_cheie in cuvinte:
                        if cuvânt_cheie.lower() in p_text.lower():
                            scor_paragraf = scor_paragraf + 2
                    
                    if scor_paragraf > 0:
                        
                        if nume == "Cod sursă public (Public code)" and ("et al." in p_text.lower() or " [" in p_text):
                            if "github" not in p_text.lower(): 
                                scor_paragraf = scor_paragraf - 3
                        
                        # Salvăm fragment
                        paragrafe_relevante_din_csv.append({
                            "text": p_text, 
                            "scor": scor_paragraf, 
                            "sectiune": rând_csv['section']
                        })

                # ALEGE CEL MAI BUN REZULTAT
                gasit, en, ro, sectiune_finala = "nu", "", "", "-"
                
                if paragrafe_relevante_din_csv:
                    # Căuta rândul cu scorul cel mai mare
                    cel_mai_bun = paragrafe_relevante_din_csv[0]
                    for p in paragrafe_relevante_din_csv:
                        if p['scor'] > cel_mai_bun['scor']:
                            cel_mai_bun = p
                    
                    gasit = "da"
                    en = cel_mai_bun["text"]
                    sectiune_finala = cel_mai_bun["sectiune"]
                    
                elif nume == "Take home message":
                    # Caută ultimul paragraf din secțiunea 'Conclusion' a fișierului CSV
                    ultimul_rând = date_specifice[date_specifice['section'] == "Conclusion"]
                    if not ultimul_rând.empty:
                        en = ultimul_rând.iloc[-1]['paragraph_text']
                        gasit = "DA (IDENTIFICAT AUTOMAT)"
                        sectiune_finala = "Conclusion"

                # Traducere IA pentru fragmentul identificat în CSV
                if gasit != "nu":
                    in_t = t_tra(en, return_tensors="pt", padding=True, truncation=True).to(dev)
                    out_t = m_tra.generate(**in_t)
                    ro = t_tra.decode(out_t[0], skip_special_tokens=True).strip()
                
                # Formatare celulă tabel
                sursa_afisaj = f"**[Sursă CSV: {sectiune_finala}]**<br>**[EN]:** {en}<br><br>**[RO]:** {ro}" if gasit != "nu" else "-"
                rezultate_temporare.append({
                    "Element Analizat": nume,
                    "Prezență": gasit.upper(),
                    "Fragment Identificat (Sursă EN + RO)": sursa_afisaj
                })
                
                # Actualizare tabel 
                df_temp = pd.DataFrame(rezultate_temporare)
                df_temp.index = range(1, len(df_temp) + 1)
                spatiu_tabel.write(df_temp.to_html(escape=False, index=True), unsafe_allow_html=True)

            spatiu_status.empty() 
            st.session_state.cache_tabel_v11 = rezultate_temporare
            st.success(f"Analiză finalizată pe baza fișierului {NUME_FISIER_SURSA}!")

        elif st.session_state.get('cache_tabel_v11'):
            df_final = pd.DataFrame(st.session_state.cache_tabel_v11)
            df_final.index = range(1, len(df_final) + 1)
            spatiu_tabel.write(df_final.to_html(escape=False, index=True), unsafe_allow_html=True)
        else:
            st.info(f"Sistem pregătit. Apăsați butonul pentru a scana {NUME_FISIER_SURSA}.")
